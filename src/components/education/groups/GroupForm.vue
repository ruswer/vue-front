  ...
  <div class="w-full lg:w-[30%] bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex-shrink-0">
    ...
    <form @submit.prevent="submitForm" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" id="name" v-model="form.name" required class="form-input text-base" />
      </div>

      <!-- Filial -->
      <div>
        <label for="branch_id" class="form-label">Filial</label>
        <select id="branch_id" v-model="form.branch_id" required class="form-select text-base">
      ...
      <div>
        ...
        <select id="status" v-model="form.status" required class="form-select text-base">
      ...
      <div>
        ...
        <select id="course_id" v-model="form.course_id" required class="form-select text-base">
      ...
      <div>
        ...
        <select id="knowledge_level_id" v-model="form.knowledge_level_id" required class="form-select text-base">
      ...
      <div>
        ...
        <select id="teacher_id" v-model="form.teacher_id" required class="form-select text-base">
      ...
      <div>
        ...
        <select id="teacher_salary_type" v-model="form.teacher_salary_type" required class="form-select text-base">
      ...
      <div>
        ...
        <div class="relative">
          <input type="number" id="teacher_salary_amount" v-model.number="form.teacher_salary_amount" required min="0" class="form-input pr-12 text-base" />
      ...
      <div>
        ...
        <select id="price_period_months" v-model="form.price_period_months" required class="form-select text-base">
      ...
      <div>
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          <div v-for="day in lessonDayOptions" :key="day.id" class="flex items-center">
            <input :id="'day-' + day.id" :value="day.id" v-model="form.lesson_days" type="checkbox" class="h-5 w-5 rounded border-gray-300 text-primary-600 focus:ring-primary-500">
            <label :for="'day-' + day.id" class="ml-2 block text-base text-gray-900 dark:text-gray-200">{{ day.label }}</label>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="relative">
          <input type="number" id="total_price" v-model.number="form.total_price" required min="0" class="form-input pr-12 text-base" />
          <span class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500">UZS</span>
        </div>
      </div>
      ...
      <div>
        ...
        <input type="number" id="lessons_count" v-model.number="form.lessons_count" required min="1" class="form-input text-base" />
      </div>
      ...
      <div>
        ...
        <div class="relative">
          <input type="number" id="price_per_lesson" v-model="form.price_per_lesson" disabled class="form-input pr-12 bg-gray-100 dark:bg-gray-600 cursor-not-allowed text-base" />
          <span class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500">UZS</span>
        </div>
      </div>
      ...
      <div>
        ...
        <input type="time" id="lesson_start_time" v-model="form.lesson_start_time" class="form-input text-base" />
      </div>

      <!-- Dars tugash vaqti -->
      <div>
        <label for="lesson_end_time" class="form-label">Dars tugash vaqti</label>
        <input type="time" id="lesson_end_time" v-model="form.lesson_end_time" class="form-input text-base" />
      </div>

      <!-- Amallar -->
      <div class="flex justify-end pt-4">
        <button type="button" v-if="groupToEdit" @click="cancel" class="mr-2 inline-flex items-center px-5 py-3 border border-gray-300 dark:border-gray-500 text-base font-medium rounded-md shadow-sm text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-600 hover:bg-gray-50 dark:hover:bg-gray-500">
          Bekor qilish
        </button>
        <button type="submit" :disabled="isSaving"
                class="inline-flex items-center px-7 py-4 border border-transparent text-lg font-bold rounded-md shadow-sm text-white bg-primary-500 hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
...
.form-input, .form-select {
  @apply mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500 px-4 py-3;
}
</style>