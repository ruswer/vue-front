  ...
  <div>
    ...
    <table class="min-w-full text-left text-sm whitespace-nowrap clear-left">
      <thead class="uppercase tracking-wider border-b-2 dark:border-neutral-600 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">
        <tr>
          <th class="px-6 py-3 cursor-pointer" @click="$emit('sort', 'name')">
            <div class="flex items-center gap-1">
              <span>Chiqim kategoriyasi</span>
              <slot name="sort-icon" :field="'name'"></slot>
            </div>
          </th>
          <th class="px-6 py-3 cursor-pointer" @click="$emit('sort', 'description')">
            <div class="flex items-center gap-1">
              <span>Tavsifi</span>
              <slot name="sort-icon" :field="'description'"></slot>
            </div>
          </th>
          <th class="px-6 py-3 cursor-pointer" @click="$emit('sort', 'status')">
            <div class="flex items-center gap-1">
              <span>Holati</span>
              <slot name="sort-icon" :field="'status'"></slot>
            </div>
          </th>
      ...
      <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
        <tr v-for="cat in paginatedCategories" :key="cat.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
          <td class="px-6 py-4 dark:text-gray-200">{{ cat.name }}</td>
          <td class="px-6 py-4 dark:text-gray-300">{{ cat.description ? cat.description.slice(0, 50) : '' }}</td>
          <td class="px-6 py-4">
            <span :class="['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium', cat.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100' ]">
              {{ cat.status === 'active' ? 'Faol' : 'Faol emas' }}
            </span>
          </td>
          <td class="px-6 py-4 text-right">
            <button type="button" @click="$emit('edit', cat)" class="inline-flex items-center justify-center w-9 h-9 rounded bg-gray-100 dark:bg-gray-700 hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-600 dark:text-blue-400 border border-gray-200 dark:border-gray-600 mr-2">
            ...
            <button type="button" @click="$emit('delete', cat.id)" class="inline-flex items-center justify-center w-9 h-9 rounded bg-gray-100 dark:bg-gray-700 hover:bg-red-100 dark:hover:bg-red-900 text-red-600 dark:text-red-400 border border-gray-200 dark:border-gray-600 ml-2">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
const emit = defineEmits(['edit', 'delete', 'sort', 'set-page', 'reset-filters'])
</script>
