<template>
  <div class="overflow-x-auto bg-white">
    <table class="min-w-full text-left text-sm whitespace-nowrap">
      <thead class="uppercase tracking-wider border-b-2 bg-gray-100 text-gray-800">
        <tr>
          <th v-for="(label, column) in columns" :key="column" class="px-6 py-4">
            {{ label }}
            <a href="#" @click.prevent="$emit('sort', column)" class="inline">
              <svg
          ...
          <th class="px-6 py-4">Tahrirlash</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr v-else v-for="student in students" :key="student.id" class="border-b">
          <td class="px-6 py-4">
            <a
              href="#"
