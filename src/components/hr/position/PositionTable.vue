    ...
    <div class="overflow-x-auto">
      ...
      <table class="min-w-full text-left text-sm whitespace-nowrap divide-y divide-gray-200 dark:divide-white/10">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-white/10">
          <tr v-for="position in filteredPositions" :key="position.id" class="hover:bg-gray-50 dark:hover:bg-white/5">
            <td class="px-6 py-4 text-gray-900 dark:text-white">{{ position.name }}</td>
            <td class="px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <button type="button" @click="$emit('edit', position)" title="Tahrirlash"
                ...
                <button type="button" @click="$emit('delete', position)" title="O'chirish"
...
const filteredPositions = computed(() => {
  ...
  if (searchModel.value) {
    result = result.filter(pos => pos.name.toLowerCase().includes(searchModel.value.toLowerCase()));
  }
  if (props.sortField) {
